#R script to reproduce Figure 3 and Figure 4 in main-text and figure S21 in the supplementary information.
#The R script written by Gaurav Baruah for the paper "Temperature tolerance and adaptive rewiring jointly predict plant–pollinator network architecture globally" - authors: Gaurav Baruah, Meike Wittmann
...
############################# TURNOVER DATA FIGURE S21 in the supplementary ############################
load("rewiring_data_20species.RData") #rewiring data for 20 species simulated plant-pollinator network 
df <- net_data %>%
  mutate(time = time * 50) #in ODE solver, time units were 50 time points apart, so this multiplication is a scaling factor
...
df2 <- df %>%
  ...
  mutate(replicate = cumsum(time == 50)) %>%
  ungroup()
# creating a dataframe of mean turnover
df_mean_turnover <- df %>%
  group_by(a, network_size, starting_temperature, competition, time) %>%
  summarise(
    mean_turnover = mean(Turnover, na.rm = TRUE), #median turnover across reps
    sd  = sd(Turnover,   na.rm = TRUE),
    n   = dplyr::n(),
    se  = sd / sqrt(40), #40 is the replicaties
    lower = mean_turnover -  1.96*se,
    upper = mean_turnover +  1.96*se,
    .groups = "drop"
  ) %>%
  # if n==1, CI is undefined; fall back to the mean (flat ribbon)
  mutate(
    lower = ifelse(is.finite(lower), lower, mean_turnover),
    upper = ifelse(is.finite(upper), upper, mean_turnover)
  )

#raw turnover over time, all reps plotteed
turnover_dyn<-ggplot(df2, 
                     aes(x     = time,
                         y     = Turnover,
                         group = replicate,
                         colour = factor(starting_temperature))) +
  ...
  scale_colour_viridis_d(option = "magma",name = expression(paste("Temperature (",degree~C,")"))) +
  theme_cowplot() +
  labs(x = "Time", y = "Turnover")

turnover_dyn


# mean turnover across replicates, so mean and and upper and lower intervals .e.e sd error plotted over time
turnover_trend <- ggplot(
  df_mean_turnover,
  aes(x = time,
      y = mean_turnover,
      colour = factor(starting_temperature),
  ...
  scale_fill_viridis_d(option = "magma", guide = "none") +
  theme_classic() +
  labs(x = "Time", y = "Mean turnover  \n across replicates")

turnover_trend #rewiring over time with mean and sd over replicates
...
df_med_turn <- df2 %>% 
  group_by(a, competition, starting_temperature, replicate) %>% #mutate(final_rewiring)
  summarise(median_turnover = median(Turnover, na.rm=TRUE),
            .groups = "drop")
...
#summarise across replicates at each temp/ treatment combo
df_trend_turnover <- df_med_turn %>%
  group_by(a, competition, starting_temperature) %>%
  summarise(
    mean_turnover = mean(median_turnover),
    sd_turnover   = sd(median_turnover),
    n_rep   = n(),
    se_rw   = sd_turnover / sqrt(n_rep),
    lower   = mean_turnover - sd_turnover,
    upper   = mean_turnover + sd_turnover,
    .groups = "drop"
  )


turnover_median<-ggplot(df_trend_turnover,
                        aes(x = starting_temperature,
                            y = mean_turnover,
                            colour = factor(a),
                            fill   = factor(a),
...
#this is figure S21 in the supplementary information
ggpubr::ggarrange(turnover_trend,turnover_median,nrow=2,
                  labels = c("A","B"))
