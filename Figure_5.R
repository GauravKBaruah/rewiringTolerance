#R script to reproduce Figure 5 in main-text, figure S2 of appendix, figure S14 of the appendix etc. The R script written by Gaurav Baruah for the paper ""Temperature tolerance and adaptive rewiring jointly predict plant–pollinator network architecture globally
rm(list=ls())
#load libraries
library(bipartite)
library(ggplot2)
...
conn<-climatic_data %>% filter(MAT > -10) %>% 
  ggplot(aes(y =Connectance , x = MAT)) +
...
nodf<-climatic_data  %>% filter(MAT > -10) %>% 
...
h2<-climatic_data  %>%  filter(MAT > -10) %>% 
...
h2
mod<-climatic_data  %>%  filter(MAT > -10) %>% 
...
coords <- read.csv("coordinates.csv")  # have Latitude and Longitude columns for all plant-poll networks compiled
head(coords)
#  world map
...
## this is the figure without the PCA, i.e., FIGURE main text in the appendix
ggpubr::ggarrange(
  r1,
